...
bool QMqttConnection::ensureTransport(bool createSecureIfNeeded)
{
...
#endif
                                   new QTcpSocket();
    //system("sudo ifconfig wlp1s0:0 192.168.0.134 up");
    //qDebug()<<m_clientPrivate->m_shostname;
  ...
  //  iface.setIPv4( "192.168.0.55");
//m_clientPrivate->m_shostname.toStdString()
    // Bring-up interface
   // iface.up();
    socket->bind(QHostAddress(m_clientPrivate->m_shostname),0,QAbstractSocket::ShareAddress);
    m_transport = socket;
    m_ownTransport = true;
    m_transportType =
#ifndef QT_NO_SSL
